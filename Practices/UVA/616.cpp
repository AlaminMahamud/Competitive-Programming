...
#define tr(container, it) \
    for (typeof(container.begin()) it = container.begin(); it != container.end(); it++) // traversal
...
void Solution :: solve(istream& cin, ostream& cout){
    ...
    while(cin >> t){
        ...
        RREP(i,n){
            ...
            MEM(flag,true);
            while(count-- && flag[i]){
                //cout << i << "=>" << count <<  "=>" << nd << endl;
                if(count==0)
                    {if(nd%i!=0) {flag[i]=false;break;}}
                else{
                    if(nd%i==1) nd -= (nd/i + 1);
                    else flag[i] = false;
                }
        ...
        if(max==-1) cout<< t <<" coconuts, no solution"<<endl;
        else cout<< t <<" coconuts, "<< max <<" people and 1 monkey"<<endl;
    }
}
